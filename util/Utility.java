...
public class Utility {
	public static Matrix3f createViewport(float worldWidth, float worldHeight, float screenWidth, float screenHeight) {
		...
		Matrix3f viewport = Matrix3f.scale(sx, -sy);
		viewport = viewport.mul(Matrix3f.translate(tx, ty));
		return viewport;
	}
	
	public static Matrix3f createReverseViewport(float worldWidth, float worldHeight, float screenWidth, float screenHeight) {
		float sx = worldWidth         / (screenWidth  - 1);
		float sy = worldHeight        / (screenHeight - 1);
		float tx = (screenWidth  - 1) / 2.0f;
		float ty = (screenHeight - 1) / 2.0f;
		Matrix3f viewport = Matrix3f.translate(-tx, -ty);
		viewport = viewport.mul(Matrix3f.scale(sx, -sy));
		return viewport;
	}
	...
	public static void fillPolygon(Graphics2D g, List<Vector2f>poly) {
		Polygon p = new Polygon();
		for(Vector2f v : poly) {
			p.addPoint((int)v.x, (int)v.y);
		}
