...
import javagames.util.XMLUtillity;
...
public class HighScoreMgr {
	...
	private void handleFileError() {
		JOptionPane.showMessageDialog(null,
				"Could not create High Score File" 
	...
	private File getHighScoreFile() {
		return new File(
		  System.getProperty("user.home"),HIGH_SCORE_FILE_NAME);
	}
	
	private void parseHighScoreFile(File file) {
		try {
			Document document = 
					XMLUtillity.parseDocument(new FileReader(file));
			Element root = document.getDocumentElement();
			for(Element player : XMLUtillity.getElements(root, "player")) {
				parsePlayer(player);
			}
		} catch(Exception e) {
			e.printStackTrace();
