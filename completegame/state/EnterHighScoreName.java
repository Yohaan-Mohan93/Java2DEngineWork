...
import javagames.util.Matrix3f;
...
public class EnterHighScoreName extends AttractState {
	...
	private int addLetters(FontMetrics fm,int start, int end, int maxWidth, Vector<GameLetter> letters) {
	...
	private int addLetter(FontMetrics fm, String letter, int max, Vector<GameLetter> letters) {
	...
	@Override
	public void render(Graphics2D g, Matrix3f view) {
		super.render(g, view);
		...
		for(int i = 0; i < letters.length; ++i) {
			...
			if(selected) {
				g.drawRect(xp, y - fm.getAscent(), maxWidth, sh);
			}
			x += maxWidth;
			count++;
			if(count % colCount == 0) {
