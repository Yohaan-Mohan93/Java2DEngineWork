...
public class GameLoading extends State {
	...
	public void enter() {
		...
		loadTasks.add(new Callable<Boolean>() {
			...
			public Boolean call() throws Exception {
				...
				Matrix3f viewport =
				 (Matrix3f)controller.getAttribute( "viewport" );
		...
		loadTasks.add(new Callable<Boolean>() {
			@Override
			public Boolean call() throws Exception{
				HighScoreMgr mrg = new HighScoreMgr();
				mrg.loadHighScores();
				controller.setAttribute("score", mrg);
				return Boolean.TRUE;
			}
		});
		loadTasks.add(new Callable<Boolean>() {
			...
			public Boolean call() throws Exception {
				...
				AsteroidFactory factory = new AsteroidFactory(wrapper,
	...
	private Element loadXML(String path) throws IOException, SAXException,
		...
		Document document = XMLUtillity.parseDocument(model);
	...
	private byte[] readBytes(InputStream in) {
		try {
			...
			ex.printStackTrace();
			new ErrorLog().createErrorLog(ex);
			return null;
		}
	}
