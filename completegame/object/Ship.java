...
public class Ship {
	...
	private Vector2f[] transformPolygon() {
		Matrix3f mat = Matrix3f.rotate(angle);
		mat = mat.mul(Matrix3f.translate(position));
	...
	private Vector2f[] transform(Vector2f[] poly, Matrix3f mat) {
	...
	public void draw(Graphics2D g, Matrix3f view) {
		if(isAlive()) {
	...
	private Particle createRandomParticle() {
		...
		switch(random.nextInt(5)) {
			...
			case 3: p.setColor( Color.ORANGE ); break;
			case 4: p.setColor( Color.PINK ); break;
		}
		int thrustAngle = 100;
		float a = (float)Math.toRadians(random.nextInt(thrustAngle) - (thrustAngle / 2));
		float velocity = random.nextFloat() * 0.375f;
		Vector2f bulletPos =
			position.add(Vector2f.polar(angle, -0.0325f));
