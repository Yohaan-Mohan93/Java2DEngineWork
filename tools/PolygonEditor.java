...
public class PolygonEditor extends SwingFramework {
	...
	private static final float BOUNDS = 0.08f;
	...
	protected void onCreateAndShowGUI() {
		...
		item = new JMenuItem(new AbstractAction("About") {
			public void actionPerformed(ActionEvent e) {
				JOptionPane.showMessageDialog(PolygonEditor.this, "About this App!!!",
	...
	private void parseModel(File file) {
		...
		try {
			...
			Document root = XMLUtillity.parseDocument(reader);
	...
	private void exportImage(File file) {
		...
		Matrix3f view = Utility.createViewport(bounds, bounds, imageWidth, imageHeight);
	...
	private void parseModel(Element model) {
		...
		polygon.clear();
		for(Element coords: XMLUtillity.getAllElements(model, "coord")) {
	...
	private void drawSprite(Graphics g) {
		...
		Matrix3f view = getViewportTransform();
	...
	private void drawPolygon(Graphics g, Matrix3f view) {
	...
	private void drawPoint(Graphics g, Matrix3f view, Vector2f v) {
		Vector2f s = view.mul(v);
		g.drawRect((int)s.x, (int)s.y, 1, 1);
	}
	
	private void drawLine(Graphics g, Matrix3f view, Vector2f v0, Vector2f v1) {
