...
public class CannonExample extends JFrame implements Runnable{
	...
	private void initialize() {
		...
		Matrix3f scale = Matrix3f.scale(0.75f, 0.75f);
	...
	private void processInput(double delta) {
		...
		if(keyboard.keyDownOnce(KeyEvent.VK_SPACE)) {
			...
			Matrix3f mat = Matrix3f.translate(7.0f, 0.0f);
			mat = mat.mul(Matrix3f.rotate(cannonRot));
			velocity = mat.mul(new Vector2f());
			
			mat = Matrix3f.translate(0.375f, 0.0f);
			mat = mat.mul(Matrix3f.rotate(cannonRot));
			mat = mat.mul(Matrix3f.translate(-2.0f, -2.0f));
